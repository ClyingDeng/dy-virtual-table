...
type SummaryMethod<T> = (data: { columns: any[]; data: T[] }) => string[]
...
type CellCls<T> = string | ((data: { row: T; rowIndex: number; column: any; columnIndex: number }) => string)
type CellStyle<T> =
  | CSSProperties
  | ((data: { row: T; rowIndex: number; column: any; columnIndex: number }) => CSSProperties)
...
interface TableProps<T> {
  ...
  tooltipOptions?: any
  spanMethod?: (data: { row: T; rowIndex: number; column: any; columnIndex: number }) =>
    | number[]
    | {
        rowspan: number
        colspan: number
